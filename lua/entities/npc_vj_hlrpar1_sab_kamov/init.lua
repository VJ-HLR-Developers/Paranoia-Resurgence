...
ENT.Model = "models/vj_parr/par1/apache.mdl"
ENT.VJ_ID_Boss = true
ENT.StartHealth = 400
...
function ENT:Init()
    ...
    local offset = math_rand(0, math_pi * 0.25)
    self.Heli_IdleMoveOffset = VJ.SET(1.7 + offset, 1.9 + offset)

    self.HeliSD_Rotor = VJ.CreateSound(self, "vj_parr/par1/apache/ap_rotor2.wav", 120)
    self.HeliSD_Whine = VJ.CreateSound(self, "vj_parr/par1/apache/ap_whine1.wav", 70)
    self.HeliSD_Distant = VJ.CreateSound(self, "vj_parr/par1/apache/ap_rotor1.wav", 160)
...
function ENT:OnUpdatePoseParamTracking(pitch, yaw, roll)
    ...
    if (math_abs(math_angledifference(self:GetPoseParameter("aim_yaw"), math_approachangle(self:GetPoseParameter("aim_yaw"), yaw, self.PoseParameterLooking_TurningSpeed))) >= self.PoseParameterLooking_TurningSpeed) or (math_abs(math_angledifference(self:GetPoseParameter("aim_pitch"), math_approachangle(self:GetPoseParameter("aim_pitch"), pitch, 20))) >= 20) then
...
function ENT:OnThink()
    ...
    if idleMoveOffset then
        ...
        ang.p = ang.p + math_sin(curTime * idleMoveOffset.a) * 0.15
        ang.r = ang.r + math_sin(curTime * idleMoveOffset.b) * 0.15
        self:SetAngles(ang)
    end
...
function ENT:OnThinkAttack(isAttacking, enemy)
    ...
    if self.Heli_HasLOS && self.Heli_NextMGT < curTime && eneData.DistanceNearest <= combatDistance && eneData.Visible && ((!self.VJ_IsBeingControlled) or (self.VJ_IsBeingControlled && self.VJ_TheController:KeyDown(IN_ATTACK))) then
...
local heliExpGibs = {
    ...
    "models/vj_parr/par1/gibs/metalplategibs_green3.mdl",
    "models/vj_parr/par1/gibs/metalplategibs_green4.mdl",
    "models/vj_parr/par1/gibs/metalplategibs_green5.mdl",
    "models/vj_parr/par1/gibs/metalplategibs_green6.mdl",
    "models/vj_parr/par1/gibs/metalplategibs_green7.mdl",
...
function ENT:OnDeath(dmginfo, hitgroup, status)
    if status == "Init" then
        ...
        expSpr:SetKeyValue("model", "vj_parr/sprites/zerogxplode.vmt")
        ...
        function deathCorpse:Initialize()
            ...
            if IsValid(phys) then
                ...
                phys:AddAngleVelocity(Vector(math_rand(-20, 20), math_rand(-20, 20), 200))
            end
        end
        ...
        function deathCorpse:Think()
            ...
            if curTime > self.NextExpT then
                ...
                local expPos2 = self:GetPos() + Vector(math_rand(-150, 150), math_rand(-150, 150), math_rand(-150, -50))
        ...
        function deathCorpse:PhysicsCollide(data, phys)
            ...
            -- Create gibs
            local gibTbl = heliExpGibs
            for _ = 1, 50 do
                local gib = ents.Create("obj_vj_gib")
                gib:SetModel(VJ.PICK(gibTbl))
                gib:SetPos(myPos + Vector(math_random(-100, 100), math_random(-100, 100), math_random(20, 150)))
                gib:SetAngles(Angle(math_rand(-180, 180), math_rand(-180, 180), math_rand(-180, 180)))
                ...
                if IsValid(myPhys) then
                    myPhys:SetMass(10)
                    myPhys:AddVelocity(Vector(math_rand(-300, 300), math_rand(-300, 300), math_rand(150, 250)))
                    myPhys:AddAngleVelocity(Vector(math_rand(-200, 200), math_rand(-200, 200), math_rand(-200, 200)))
                end
            ...
            local expPos2 = myPos + Vector(0, 0, math_rand(150, 150))
            local spr = ents.Create("env_sprite")
            spr:SetKeyValue("model", "vj_parr/sprites/fexplo1.vmt")
            spr:SetKeyValue("GlowProxySize", "2.0")
            spr:SetKeyValue("HDRColorScale", "1.0")
            spr:SetKeyValue("renderfx", "14")
            spr:SetKeyValue("rendermode", "5")
            ...
            spr:Spawn()
            spr:Fire("Kill", "", 1)
            timer.Simple(1, function() if IsValid(spr) then spr:Remove() end end)
