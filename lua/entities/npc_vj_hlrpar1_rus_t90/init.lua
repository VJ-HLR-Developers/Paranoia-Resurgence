    ...
    for i = 0, 1, 0.5 do
        timer.Simple(i, function()
            if IsValid(self) then
                ...
                spr:SetPos(myPos + self:GetForward() * -10 + expPos)
                spr:Spawn()
...
function ENT:Tank_OnDeathCorpse(dmginfo, hitgroup, corpse, status, statusData)
    if status == "Override" then
        ...
        util.BlastDamage(self, self, myPos + myForward * -10 + myUp * 80, 200, 10)
        util.ScreenShake(myPos, 100, 200, 1, 2500)
        ...
        if math_random(1, self.Tank_DeathDriverCorpseChance) == 1 then
            ...
            if soldierMDL then
                self:CreateExtraDeathCorpse("prop_ragdoll", soldierMDL, {Pos = myPos + myForward * -10 + myUp * 90 + self:GetRight() * -30, Vel = Vector(math_rand(-600, 600), math_rand(-600, 600), 500)}, function(ent)
        ...
        spr:SetKeyValue("spawnflags", "0")
        spr:SetKeyValue("scale", "2")
        spr:SetPos(myPos + myForward * -10 + expPos)
        spr:Spawn()
        spr:Fire("Kill", nil, 0.9)
        timer.Simple(0.9, function() if IsValid(spr) then spr:Remove() end end)
        ...
        fireSpr:SetKeyValue("scale", "2")
        fireSpr:SetPos(myPos + myForward * -10 + myUp * 100)
